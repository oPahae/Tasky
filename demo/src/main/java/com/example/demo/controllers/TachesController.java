...
public class TachesController {
   ...
   public void addTache(@RequestBody Tache tache){
    ...
    public List<Blocage> getBlockagesByTacheId(@PathVariable int id){
        Tache t=tacheRepository.findById(id).orElse(null);
        if(t!=null){
            return t.getBlocages();
        }
        return null;
    }
    ...
    public boolean finirTache(@PathVariable int id){
        ...
        if(t!=null && staches.size()>0){
           if(staches.size()==staches.stream().filter(SousTache::isTermine).count()){
           t.setEtat("Terminé");
    ...
    @PutMapping("/taches/{id}")
     ...
     public void bloquerTache(@PathVariable int id,@RequestBody  Blocage blockage){
    Tache t=tacheRepository.findById(id).orElse(null);
        if(t!=null){
             List<Blocage> blockages=t.getBlocages();
             blockages.add(blockage);
             t.setBlocages(blockages);
             t.setEtat("Bloquée");
             tacheRepository.save(t);
       }
    }
    ...
    public boolean tacheCloturer(@PathVariable int id){
       Tache st=tacheRepository.findById(id).orElse(null);
         if(st!=null && st.getDateFin()!=null){
             if(LocalDate.now().isAfter(st.getDateLimite()) && !st.getEtat().equals("terminée")){
